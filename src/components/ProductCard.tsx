...
export function ProductCard({ product, onBuy, loading = false }: ProductCardProps) {
  return (
    <div className="bg-brand-bg border border-brand-border rounded-3xl p-8 hover:border-brand-accent transition-all hover:shadow-lg hover:shadow-brand-accent/10">
      ...
      <p className="mt-2 text-brand-textDark/60 text-sm">{product.description}</p>
      <ul className="mt-4 space-y-1">
        {product.specs.map((s) => (
          <li key={s} className="text-brand-textDark/80 text-sm flex items-center gap-2">
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="px-6 py-2 bg-brand-accent hover:bg-brand-accentHover disabled:opacity-50 text-white rounded-full text-sm font-medium transition-all"
        >
          {loading ? '処理中...' : '購入する'}
        </button>
      </div>
